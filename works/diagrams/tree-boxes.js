...
function treeBoxes(urlService, jsonData)
{
	...
	var rectNode = { width : 163, height : 63, textMargin : 5 },
...
*/
	...
	// 1) ******************* Update the nodes *******************
		...
		.attr('width', tooltip.width*2)
		.attr('height', tooltip.height*8)
    	...
    	.attr('x', rectNode.width / 2 + tooltip.textMargin*2)
		...
		.style('fill', 'white')
		.append("tspan")
	    .text(function(d) {return 'Название: ' + d.name;})
	    .append("tspan")
	    .attr('x', rectNode.width / 2 + tooltip.textMargin)
	    .attr('dy', '1.5em')
	    .text(function(d) {return 'Информация: ' + d.label;});
		...
		nodeUpdate.select('text').style('fill-opacity', 1).style({
          "fill": function(d) {
            if (d.children && d.parent) {
              return yellow2;
            }
